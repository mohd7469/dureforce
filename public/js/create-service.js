...
/*!*******************************************************!*\
  !*** ./assets/resources/js/service/create-service.js ***!
  \*******************************************************/
...
function overviewFormValidation() {
  $(".user-profile-form").submit(function (e) {
    ...
    if ($('#service_features :selected').length <= 0) {
      e.preventDefault();
      $(".include_error").after('<span class="error text-danger">This field is required</span>');
      iziToast.error({
  ...
  $(".banner-form").submit(function (e) {
    ...
    if ($("#pages div#banner2").css("display") == "none") {
      if ($("#static_banner_image").val().length < 1 && !$("input[name='image']").attr("value")) {
        ...
        $("#static_banner_image").after('<span class="error text-danger" style="margin-top:5px !important;">This field is required</span>');
        iziToast.error({
...
function validateAddOnRows(element, e) {
  if (element.find(".add-on-delivery").val() != '' && element.find(".add-on-delivery").val() < 1) {
    e.preventDefault();
    element.find(".add-on-delivery").after('<span class="error text-danger">Field value should be greater than zero</span>');
  }
}
