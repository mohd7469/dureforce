...
/*!******************************************************************!*\
  !*** ./assets/resources/templates/basic/frontend/js/job.view.js ***!
  \******************************************************************/
...
var genRand = function genRand(len) {
...
function populateSkills(data) {
  ...
  for (var main_category in data) {
    ...
    for (var sub_category_enum in all_sub_categories) {
      //front end backend 

      var skills = all_sub_categories[sub_category_enum];
      var sub_category_id = genRand(5);
      var sub_skills = skills.map(function (a) {
        return a.id;
      });
      if (selected_skills.some(function (r) {
        return sub_skills.includes(r);
      })) {
        remove = false;
    ...
    if (remove) $('#' + main_category_id).remove();
  }
...
function fetchSkills() {
  ...
  $.ajax({
    ...
    data: {
      ...
      sub_category_id: sub_catgory_id
    },
