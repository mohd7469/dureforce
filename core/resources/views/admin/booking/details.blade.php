...
@section('panel')
        ...
        <div class="col-xl-4 col-md-6 mb-30">
            <div class="card b-radius--10 overflow-hidden box--shadow1">
                <div class="card-body">
                    ...
                    <h6 class="mb-20 text-muted"><a
                            href="{{ !empty($booking->service_id) ? route('admin.service.details', $booking->service_id) : '#' }}">{{ __(@$booking->service->title) }}</a>
                    </h6>
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span class="font-weight-bold">
                                <a
                                    href="{{ !empty($booking->service->user_id) ? route('admin.users.detail', $booking->service->user_id) : '#' }}">{{ @$booking->service->user->username }}</a>
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Buyer')
                            <span class="font-weight-bold">
                                <a
                                    href="{{ route('admin.users.detail', $booking->user_id) }}">{{ @$booking->user->username }}</a>
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Delivery Data')
                            <span
                                class="font-weight-bold">{{ showDateTime($booking->created_at->addDays($booking->service->delivery_time ?? 5), 'd M Y') }}</span>
                        </li>

                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Service Price')
                            <span class="font-weight-bold">{{ getAmount($booking->service->price ?? 0) }}
                        ...
                        @if ($booking->discount != 0)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @lang('Discount')
                                <span
                                    class="font-weight-bold">{{ __($general->cur_sym) }}{{ showAmount($booking->discount ?? 0) }}</span>
                            </li>
                        @endif

                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Amount')
                            <span class="font-weight-bold">{{ getAmount($booking->amount ?? 0) }}
                                            ...
                                            @foreach (explode(',', $booking->extra_service) as $value)
                                                ...
                                                <tr>
                                                    ...
                                                    <td data-label="@lang('Price')"><span
                                                            class="font-weight-bold">{{ $general->cur_sym }}{{ showAmount($extraService->price) }}</span>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                        ...
                        <div class="col-md-12 mt-2">
                            @foreach ($booking->workFile as $value)
                                <div class="card-title text-center font-weight-bold">
                                    <a href="{{ route('admin.work.delivery.download', encrypt($value->id)) }}"
                                        class="icon-btn btn--primary"><i class="las la-download"></i>
                                    </a>
                                </div>
    ...
    <div class="modal fade" id="disputeModalShow" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
...
@push('breadcrumb-plugins')
    @if ($booking->status == 2)
        <button class="btn btn--success ml-1 approveBtn" data-toggle="tooltip" data-id="{{ $booking->id }}"
            data-original-title="@lang('Seller Payment')"><i class="fas fa-ban"></i>
            @lang('Seller Payment')
        </button>

        <button class="btn btn--info ml-1 refundBtn" data-toggle="tooltip" data-id="{{ $booking->id }}"
            data-original-title="@lang('Buyer Payment')"><i class="fas fa-ban"></i>
            @lang('Buyer Payment')
        </button>
    @endif
@endpush
