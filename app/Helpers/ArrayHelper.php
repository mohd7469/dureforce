...
{
    public const
        RETURN_KEY = 1, // used in inArrayExt. Returns key in matching case else false.
        RETURN_BOOL = 2; // used in inArrayExt. Returns true if matched else false.
    ...
    /**
     * Supports different return types as well as can trim or lower or upper haystack value (if string) when matching.
     *
     * @param mixed $needle
     ...
     * @return mixed
     */
    public static function inArrayExt($needle, array $haystack, int $returnType = self::RETURN_KEY, int $case = StringHelper::NO_CASE, bool $trim = false)
    {
        foreach ($haystack as $key => $value) {
            if ($trim !== false || $case !== StringHelper::NO_CASE) {
                $value = StringHelper::to($value, $case, $trim);
            };
            if ($value === $needle) {
                switch ($returnType) {
    ...
    public static function isNotArray($array): bool
    {
        return !static::isArray($array);
    }
    ...
    public static function stringToArray(string $array, string $delimiter = '-'): array
    {
    ...
    public static function allNumeric(array $array): bool
    {
    ...
    /**
     * AntD's select box needs data in certain format.
     * @param array $array
     * @return array
     */
    public static function toIdNameMap(array $array): array
    {
        return collect($array)
            ->map(function ($value, $key) {
                return ['id' => $key, 'name' => $value];
            })
            ->values()// keys will be reset. imp. associative arr will be considered as obj in JS.
            ->all();
    }

    public static function changeKeyCase(array $array, int $case = CASE_LOWER)
    {
