...
class OffersController extends Controller
{
    public function  index(){
        ...
        if ( $last_role_id  == Role::$Freelancer ) {
            $offers=ModuleOffer::with('module')->where('offer_send_to_id','=',$user->id)->where('is_active',1)->paginate(getPaginate());
            $total = count($offers);
        }
        else if( $last_role_id == Role::$Client ){
            $offers=ModuleOffer::with('module')->where('offer_send_by_id','=',$user->id)->where('is_active',1)->paginate(getPaginate());
            $total = count($offers);
        }
