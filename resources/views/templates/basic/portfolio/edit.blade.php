...
<div class="row section-heading-border " style="margin-bottom: 23px;">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12 col-lg-12">
        ...
        @php
            $files=old('uploaded_files',$user_portfolio->attachments);
            if(is_string($files)){
        ...
        <form action="{{route('seller.profile.portfolio.store')}}" id="portfolio_information_form" method="post">
            @csrf
            <input type="hidden" value="{{$user_portfolio->id}}" name="id" >
            {{-- Name --}}
            <div class="col-md-12 col-lg-12 form-group">
                <label for="exampleInputEmail1">Project Name *</label>
                <input type="text" class="form-control" id="project_name" aria-describedby="emailHelp" placeholder="Enter Project Name" name="name" value="{{old('name',$user_portfolio->name)}}">
            </div>
            {{-- description --}}
            <div class="col-md-12 col-lg-12 form-group">
                <label>Description</label>
                <textarea class="" name="description"  rows="4"  placeholder="Enter Project Description">{{old('description',$user_portfolio->description)}}</textarea>
            </div>

            {{-- Role --}}
            <div class="col-md-12 col-lg-12 form-group">
                <label>What was your Role in the Project?</label>
                <textarea class="" name="role"  rows="4"  placeholder="Write about your role in this project.">{{old('role',$user_portfolio->role)}}</textarea>
            </div>
                    ...
                    @if ($user_portfolio->attachments->isNotEmpty())
                        @foreach ($user_portfolio->attachments as $item)
                            <div class="col-md-6 col-sm-12 col-xs-12 col-lg-4">
                                <span class="text-right user-portfolio-delete-btn"  style="position: absolute;top: -10px;right: -3px;"  data-file="{{$item->id}}">
                                    <i class="fa fa-times-circle" style="color:#007F7F;margin-right: 10px;"></i>
                                </span>
                                <img alt="User Pic" src="{{ $item->url }}" id="profile-image1" class="image-files img-responsive" alt="Portfolio image not found">    
                            </div>
                        @endforeach 
                    @endif
            
                    <div class="col-md-6 col-sm-12 col-xs-12 col-lg-4" id="dropzone_div">
                        <div id="dropzone">
                            <div class="dropzone needsclick dz-clickable" id="edit_portfolio-dropzone-upload" >
                ...
                <div class="input-group mb-1 select2_element">
                        <select
                            ...
                            @foreach($skills as $skill)
                                <option value="{{$skill->id}}"  {{ in_array($skill->id,old('skills',$user_portfolio->skills->pluck('id')->toArray())) ? 'selected' : ''}}>{{$skill->name}}</option>
                            @endforeach
            ...
            <div class="row mt-2">
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-xs-12 form-group">
                    ...
                    <input type="text" name="project_url" id="detail_project_url" class="form-control" value="{{old('project_url',$user_portfolio->project_url)}}">
                </div>
                
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-xs-12 form-group"> 
                    <label>Youtube URL</label>     
                    <input class="form-control video-url" id="video_url_id" type="text" placeholder="Add Video Link"  name="video_url" value="{{old('video_url',$user_portfolio->video_url)}}">
                </div>
            </div>

            <input type="hidden" value="{{old('uploaded_files',json_encode($user_portfolio->attachments))}}" name="uploaded_files" id="uploaded_files_id">
...
@push('script-lib')
    ...
    <script>
        ...
        uploaded_files= JSON.parse($('#uploaded_files_id').val());
        ...
        $(document).ready(function(){
            ...
            var dropzone = new Dropzone('#edit_portfolio-dropzone-upload', {
                            url:action_url,
                            ...
                            init: function() {
                                ...
                                this.on("success", function (file, response_data) {
                                    var _this = this;
                                    _this.removeFile(file);
                                    uploaded_files.push(response_data.uploaded_file);
                                    addFile(response_data.uploaded_file);
                                });
        ...
        $(document).on('click', '.user-portfolio-delete-btn', function() {
            file_id=$(this).data('file');
            let result = uploaded_files.filter(function (obj) {
                return obj.id != file_id;
            });
            $('#uploaded_files_id').val(JSON.stringify(result));
            $(this).parent().remove();
        });
        function addFile(file){

           
            $('#uploaded_files_id').val(JSON.stringify(uploaded_files));
            $('#dropzone_div').parent().prepend(`
                                <div class="col-md-6 col-sm-12 col-xs-12 col-lg-4">
                                    <span class="text-right"  style="position: absolute;top: -10px;right: -3px;">
                                        <i class="fa fa-times-circle" style="color:#007F7F;margin-right: 10px;"></i>
                                    </span>
                                    <img alt="User Pic" src="`+file.url+`" id="profile-image1" class="image-files img-responsive" alt="Portfolio image not found">    
                                </div>
                            `);
        }
