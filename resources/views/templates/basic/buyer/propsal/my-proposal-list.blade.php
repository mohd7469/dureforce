 ...
 <div class="tab-content">
        <div class="listing_table_con card-body tab-pane active" id="all"> 
            <table>
                ...
                <tr>
                <td>
                    <h2 class="per_heading">{{isset($proposal->job) ? $proposal->job->title: null}}</h2>
                    <p class="per_jobs_d">{{ __(str_limit($proposal->job->description ?? '', 50)) }}</p>
                </td>
                <td>
                    <span class="jobtype-per">Service</span>
                </td>
                <td>
                    <p class="job_price">{{$proposal->amount_receive ?? ''}}</p>
                </td>
                <td><span class="job_status_p">Draft</span></td>
                <td><a href="{{ route('seller.proposal.detail',$proposal->uuid) }}" class="view_propasal_per">View Proposal</a></td>
                </tr>
                @endforeach
                @endisset
            </table>
