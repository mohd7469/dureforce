    ...
    <div class="sidebar">
        ...
        <div class="widget mb-30">
            ...
            <div class="accordion " id="accordionExample">
                @foreach(\App\Models\Category::groupByType() as $index=> $c)
                    <div class="accordion-item mb-2">


                        <button class="accordion-button collapsed mb-2" type="button" data-bs-toggle="collapse"
                                data-bs-target="#{{$index}}" aria-expanded="false" aria-controls="{{$index}}">
                            {{$index}}
                        </button>
                        <div id="{{$index}}" class="accordion-collapse collapse border-0 " aria-labelledby="headingOne"
                             data-bs-parent="#accordionExample">

                            <ul class="category-list">
                                {{--                {{dd(\App\Models\Category::groupByType())}}--}}
                                @foreach($c as $category)
                                    <li>
                                        <a href="{{route('job.category', [slug($category->name),$category->id])}}">{{__($category->name)}}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
