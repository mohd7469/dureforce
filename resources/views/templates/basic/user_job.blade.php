                                                ...
                                                <div class="profile-widget">
                                                    <div class="profile-widget-header">
                                                        <div class="profile-widget-thumb">
                                                            <img src="{{ userDefaultImage(imagePath()['profile']['user_bg']['path'] . '/' . $user->bg_image, 'background_image') }}"
                                                                alt="item-banner">
                                                            @if ($user->isOnline())
                                                                <span
                                                                    class="badge-offline bg--success">@lang('Online')</span>
                                                            @else
                                                                <span
                                                                    class="badge-offline bg--warning">@lang('Offline')</span>
                                                            @endif
                                                            @if (auth()->check())
                                                        ...
                                                        <div class="profile-widget-author">
                                                            <div class="thumb">
                                                                <img src="{{ userDefaultImage(imagePath()['profile']['user']['path'] . '/' . $user->image, 'profile_image') }}"
                                                                    alt="{{ __($user->username) }}">
                                                            </div>
                                                            <div class="content">
                                                                <h4 class="name">
                                                                    <a
                                                                        href="{{ route('profile', $user->username) }}">{{ __($user->username) }}</a>
                                                                </h4>
                                                                <span
                                                                    class="designation">{{ @$user->designation }}</span>
                                                                <div class="ratings">
                                                                    <span><i class="las la-star text--warning"></i>
                                                                        {{ getAmount($reviewAvg, 2) }} ({{ $reviewCount }}
                                                                        @lang('reviews'))</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="profile-list-area">
                                                        <ul class="details-list">
                                                            <li><span>@lang('Total Service')</span>
                                                                <span>{{ $user->services->count() }}</span></li>
                                                            <li><span>@lang('Work Complete')</span>
                                                                <span>{{ $workCompleteCount }}</span></li>
                                                            <li><span>@lang('Total Software')</span>
                                                                <span>{{ $user->softwares->count() }}</span></li>
                                                            <li><span>@lang('Work Pending')</span>
                                                                <span>{{ $workPendingCount }}</span></li>
                                                            <li><span>@lang('Total Job')</span>
                                                                <span>{{ $user->jobs->count() }}</span></li>
                                                            <li><span>@lang('Country')</span>
                                                                <span>{{ @$user->address->country }}</span></li>
                                                                    ...
                                                                    <div class="item-card-content-top">
                                                                        ...
                                                                        <div class="right">
                                                                            <div class="item-amount">
                                                                                {{ $general->cur_sym }}{{ showAmount($job->amount) }}
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                ...
                                                                <div class="item-card-footer">
                                                                    <div class="left">
                                                                        <a href="javascript:void(0)"
                                                                            class="btn--base active date-btn">{{ __($job->delivery_time) }}
                                                                            @lang('Days')</a>
                                                                        <a href="{{ route('job.details', [slug($job->title), encrypt($job->id)]) }}"
                                                                            class="btn--base bid-btn">@lang('Total
                                                                            Bids')({{ $job->jobBiding->count() }})</a>
                                                                    </div>
                                                                    <div class="right">
                                                                        <div class="order-btn">
                                                                            <a href="{{ route('job.details', [slug($job->title), encrypt($job->id)]) }}"
