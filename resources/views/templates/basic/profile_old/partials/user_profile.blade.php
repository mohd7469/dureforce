        ...
        <div class="container-fluid welcome-body px-5">
            ...
            <div>
                ...
                <div class="profile-img col-md-12" action="">
                    ...
                    @if (auth()->user()->image)
                        <image width="100" height="100" id="preview-img-edit"
                            src="{{ asset('assets/images/user/profile') . '/' . auth()->user()->image }}" />
                    @endif

                </div>
                <div class="col-md-12">
                    <p style="font-size: 12px" id="img_error">Drop file here or Browse to add</p>
                </div>
                <div class="col-md-12">
                    <label class="mt-4">Job Title <span class="imp">*</span></label>
                    <input type="text" name="designation" id = "title" placeholder="E.g. Full Stack Developer"
                        value="{{ auth()->user()->designation ?? '' }}" >
                </div>
                <div class="col-md-12">
                    <label class="mt-4">About You <span class="imp">*</span></label>
                    <textarea  cols="20" rows="5" name="about_me"
                        placeholder="Describe yourself to clients" id="about">{{ auth()->user()->about_me ?? '' }}</textarea>
                </div>
                <div class="col-md-12">
                    <label class="mt-4">Location <span class="imp">*</span></label>
                    <input type="text" name="location" value="{{ auth()->user()->address->address ?? '' }}"
                        placeholder="City, Country" id="location" />
                </div>
                <div class="col-md-12">
                    <label class="mt-4">Phone <span class="imp">*</span></label>
                    <input type="number" name="mobile" placeholder="" id="phone" value="{{ auth()->user()->mobile ?? '' }}"
                         />
                </div>
                    ...
                    @forelse(auth()->user()->languages as $langKey => $language)
                        <div class="row" id="language-row-{{ $langKey }}" style="align-items: center;">
                            ...
                            <div class="col-md-6 col-sm-10">
                                ...
                                <select name="languages[]" class="form-control select-lang" id="languages" >
                    ...
                    @empty
                        ...
                        <div class="col-md-6 col-sm-12">
                            <label class="mt-4">Profeciency Level <span
                                    class="imp">*</span></label>
                            <select name="language_level[]" class="form-control selected-level select-lang" id="langLevel"
                                >
                                <option value="" selected>
                                    My Level is
                                </option>
