@foreach ($attributes as $key => $attr)
    @php 
        ...
        if(! empty($model->softwareDetail->entity_fields)) {
            if(array_key_exists($key, $model->softwareDetail->entity_fields)) {
                $check = $model->softwareDetail->entity_fields[$key];
            }
        }
    @endphp  
    ...
    <div class="row">
            ...
            @if ($attr->field_type != true)
                <select name="entity_field[]" class="form-control {{count($attr->attributes) > 1 ? 'attribute-selector' : ''}}" id=""
                    ...
                    <option
                        {{ $check == '1' ? 'selected' : '' }}
                        value="1">
                        {{ $attr->field_one ?? 'Front End' }}</option>
                    <option value="0"
                        {{ $check == '0' ? 'selected' : '' }}>
                        {{ $attr->field_two ?? 'Back End' }}</option>
        ...
        <div class="{{ $attr->attribute_type == true ? 'attributes' : 'attributes-radio' }}">
                <div class="col-xl-12 col-lg-12 form-group mt-2 d-flex flex-wrap back"
                    style="display: {{ $check == '1' ? 'none !important' : '' }}"
                    id="back-{{ $key }}">
                        ...
                        @if ($back->type == 0)
                            <div class="form-group custom-check-group px-2">
                                <input class="{{ $attr->attribute_type == true ? 'attrs-back' : 'attrs-radio-back' }}" type="{{ $attr->attribute_type == true ? 'checkbox' : 'radio' }}"
                                    @foreach ($model->softwareAttributes as $val) {{ $back->id == $val->attribute_id ? 'checked' : '' }} @endforeach
                ...
                <div class="col-xl-12 col-lg-12 form-group mt-2 d-flex flex-wrap front"
                    style="display: {{ $check == '0' ? 'none !important' : '' }}"
                    id="front-{{ $key }}">
                        ...
                        @if ($front->type == 1)
                            <div class="form-group custom-check-group px-2">
                                <input class="{{ $attr->attribute_type == true ? 'attrs-front' : 'attrs-radio-front' }}" type="{{ $attr->attribute_type == true ? 'checkbox' : 'radio' }}"
                                    @foreach ($model->softwareAttributes as $val) {{ $front->id == $val->attribute_id ? 'checked' : '' }} @endforeach
