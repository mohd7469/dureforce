...
<div class="row">
    <div class="col-lg-12">
        <a href="{{ route('admin.leadImages.create') }}" class="btn btn-primary btn-sm float-right">Create Leading image</a>
    </div>
</div>
<br>
<div class="row">
                    ...
                    <table class="table table--light style--two">
                        ...
                        <tr>
                            ...
                            <td data-label="@lang('Action')">
                                ...
                                @endif
                                <a  href="{{route('admin.leadImages.edit', $banner->id)}}" class="icon-btn btn--success ml-1  editbtn-c" id="" data-toggle="tooltip1" title="" data-original-title="@lang('Edit')" data-id="">
                                        <i class="las la-edit"></i>
                                </a>
                                <td>
                                    <form action="{{route('admin.leadImages.destroy', [$banner->id])}}" method="POST" >
                                        @csrf
...
<div class="modal fade" id="inactiveBy" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.leadImages.inactiveBy') }}" method="POST">
                @csrf
                @method('POST')
                <input type="hidden" name="id">
                <div class="modal-body">
                    <p>@lang('Are you sure to inactive this Lead image post?')</p>
                </div>
...
<div class="modal fade" id="activeBy" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.leadImages.activeBy') }}" method="POST">
                @csrf
                @method('POST')
                <input type="hidden" name="id">
                <div class="modal-body">
                    <p>@lang('Are you sure to active this Lead image post?')</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn--secondary" data-dismiss="modal">@lang('Close')</button>
                    <button type="submit" class="btn btn--success">@lang('Confirm')</button>
